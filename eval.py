...
parse.add_argument('--output', type=str, default='/home/ouc/TXH/MyCode/PointNet++/save_model', help='Path for saving model')
...
model.load_state_dict(torch.load('/home/ouc/TXH/MyCode/PointNet++/save_model_msg/39.pth'))
model.eval()
total_point_num = 0
correct_point_num = 0
with torch.no_grad():
    for idx, data in enumerate(test_loader):
        ...
        weight = torch.Tensor(test_dataset.label_weight).to(device)
        ...
        correct_point_num += correct.item()
        total_point_num += float(target.size()[0] * target.size()[1])
        correct_per = correct.item() / float(target.size()[0] * target.size()[1])
        print('loss:%7f, correct:%7f ' % (loss.item(), correct_per))
        # Error 20:ZeroDivisionError: integer division or modulo by zero
print('All correct percent:{}'.format(correct_point_num / total_point_num))
# All correct percent:0.7910495051078714 epoch 20 shuffle=False
#
